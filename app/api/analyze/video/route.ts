...
async function downloadVideo(url: string): Promise<string> {
  ...
  try {
    ...
    // Adding user-agent and no-check-certificate to avoid 403 errors
    // Using --output option without extension to let yt-dlp choose the correct one
    execSync(`yt-dlp --no-check-certificate --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36" -o "${baseOutputPath}" "${url}"`, { stdio: 'pipe' });
    ...
    try {
      ...
      execSync(`yt-dlp --no-check-certificate --extractor-args "youtube:player_client=android" -o "${baseOutputPath}" "${url}"`, { stdio: 'pipe' });
...
async function extractAudio(videoPath: string): Promise<string> {
  ...
  try {
    ...
    // Extract audio using ffmpeg, convert to 16kHz mono WAV for Google Speech API
    // Use quotes around paths to handle spaces in filenames
    execSync(`ffmpeg -i ${videoPath} -vn -acodec pcm_s16le -ar 16000 -ac 1 ${outputPath}`, { 
      stdio: 'pipe',
      encoding: 'utf-8',
      maxBuffer: 10 * 1024 * 1024 // Increase buffer size to handle larger outputs
    });
...
async function splitAudioToChunks(audioPath: string, chunkDurationMs: number = 60000): Promise<string[]> {
  ...
  try {
    ...
    const durationOutput = execSync(`ffprobe -i "${audioPath}" -show_entries format=duration -v quiet -of csv="p=0"`, { encoding: 'utf-8' });
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      execSync(`ffmpeg -i "${audioPath}" -ss ${startTime} -t ${chunkDurationMs/1000} -c copy "${chunkPath}"`, { stdio: 'pipe' });
...
async function analyzeTranscript(transcript: { english: string; hindi: string }): Promise<any> {
  try {
    ...
    const prompt = `
      Analyze the following transcript from a video in both English and Hindi and assess its radicalism level:
      ...
      - Lexical Analysis: Use of radical terminology, exclusionary language, or divisive rhetoric
      - Emotion and Sentiment: Use of fear, anger, or other emotions to manipulate
      - Speech Patterns and Intensity: Tone, urgency, repetition of concepts
      - Use of Religious Rhetoric: How religious symbolism is used to justify positions
      - Frequency of Commands and Directives: Calls to action
    ...
    let radicalProbability = 50; // Default values if extraction fails
    ...
    // Get score label and color based on the scores
    ...
    const riskFactors = extractRiskFactors(analysisText);
    
    // Create a list of safety tips based on the analysis
    const safetyTips = generateSafetyTips(radicalProbability, radicalContent);
    
    // Generate overall assessment
    const overallAssessment = `This content has been analyzed for radical elements with an overall concern level of ${scoreLabel.toLowerCase()}. The radical probability score is ${radicalProbability}/100, and the radical content score is ${radicalContent}/100.`;
...
function extractRiskFactors(text: string): string[] {
  // Look for list items or key phrases in the analysis
  const factors = [];
  ...
  // If still no factors, create generic ones based on scores
  if (factors.length === 0) {
...
async function processVideoUrl(url: string): Promise<any> {
  try {
    ...
    // Step 4: Transcribe each chunk and combine
    ...
    for (let i = 0; i < chunkPaths.length; i++) {
      ...
      // Parallel transcription in both languages
    ...
    // Could improve this by extracting actual thumbnail and duration using ytdl info
    const videoDuration = 0; // Default duration
